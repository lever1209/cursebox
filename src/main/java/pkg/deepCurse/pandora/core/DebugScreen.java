...
public class DebugScreen extends Screen {
	...
	public void init(){
		...
		new TranslatableText("pandora.menu.debug.save.config"), (buttonWidget) -> {
			PandoraConfig.saveConfigs();
		}));
		this.addDrawableChild(new ButtonWidget(0, 20, 59, 10,
		new TranslatableText("pandora.menu.debug.reload.config"), (buttonWidget) -> {
			PandoraConfig.loadConfig();
		}));
		this.addDrawableChild(new ButtonWidget(0, 30, 69, 10,
		new TranslatableText("pandora.menu.debug.delete.config"), (buttonWidget) -> {
			PandoraConfig.deleteConfig();
		}));
		this.addDrawableChild(new ButtonWidget(0, 40, 79, 10,
				new TranslatableText("pandora.menu.debug.unpack.config"), (buttonWidget) -> {
					try {
		...
		this.addDrawableChild(new ButtonWidget(0, 50, 79, 10,
				new TranslatableText("pandora.menu.debug.new.config.instance"), (buttonWidget) -> {
					try {
